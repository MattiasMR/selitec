document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (href.includes('wa.me') || href.includes('whatsapp')) {
            trackEvent('click_whatsapp_cta', { destination: href });
        }

        if (href.startsWith('tel:')) {
            trackEvent('click_phone_cta', { destination: href });
        }

        if (link.hasAttribute('download')) {
            trackEvent('download_pdf', { destination: href });
        }
    });
    ...
    if (coursesGrid) {
        ...
        const renderCourses = (courses) => {
            ...
            courses.forEach(course => {
                ...
                card.innerHTML = `
                    <img src="${imgSrc}" alt="${course.title}" class="course-card__image" width="300" height="200" loading="lazy">
                    ...
                    <div class="course-card__footer">
                        <a href="${courseUrl}" class="btn btn--outline btn--full">Ver Temario</a>
                    </div>
                `;
